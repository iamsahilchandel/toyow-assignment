...
import { DAGDefinition, dagDefinitionSchema } from './workflows.types';
import { workflowValidationService } from './workflowValidation.service';
...
export class WorkflowService {
  ...
  async createWorkflow(
    ...
    workflowValidationService.validate(validatedDag);
   ...
   */
  async getVersions(workflowId: string, userId: string, userRole: string) {
    // Verify access
    await this.getWorkflow(workflowId, userId, userRole);

    const versions = await prismaClient.workflowVersion.findMany({
      where: { workflowId },
      orderBy: { version: 'desc' },
    });
   ...
   */
  async getVersion(workflowId: string, versionId: string, userId: string, userRole: string) {
    // Verify access
    await this.getWorkflow(workflowId, userId, userRole);
